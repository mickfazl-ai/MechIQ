...
function ServiceTab({ asset }) {
  ...
  const loadCompleted = async () => {
    ...
    const { data } = await supabase
      .from('service_sheet_submissions')
      .select('*')
  ...
  useEffect(() => {
    if (serviceSubTab === 'completed') loadCompleted();
  }, [serviceSubTab]);
  ...
  const exportCompletedPDF = () => {
    const rows = completed.map(s => `
      <tr>
        ...
        <td>${s.service_type || s.template_id || '—'}</td>
        <td>${s.technician || '—'}</td>
        <td>${s.total_labour_hours ? s.total_labour_hours + ' hrs' : '—'}</td>
        <td>${s.notes || '—'}</td>
      </tr>`).join('');
    const win = window.open('', '_blank');
    win.document.write(`
      <!DOCTYPE html><html><head><title>Service History — ${asset.name}</title>
      <style>
        ...
        th { background: #f1f5f9; padding: 9px 12px; text-align: left; font-size: 10px; text-transform: uppercase; letter-spacing: .5px; color: #64748b; border-bottom: 2px solid #e5e7eb; }
        td { padding: 9px 12px; border-bottom: 1px solid #f1f5f9; vertical-align: top; }
        tr:last-child td { border-bottom: none; }
        .footer { margin-top: 32px; font-size: 10px; color: #94a3b8; }
      </style></head><body>
      <h1>Service History — ${asset.name}</h1>
      <p>Asset: ${asset.asset_number || asset.name} · Exported: ${new Date().toLocaleDateString('en-AU')} · ${completed.length} records</p>
      <table>
        <thead><tr><th>Date</th><th>Service Type</th><th>Technician</th><th>Labour</th><th>Notes</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
        ...
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
            ...
            <button onClick={exportCompletedPDF}
              style={{ padding:'7px 14px', background:'var(--accent)', color:'#fff', border:'none', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:5 }}>
              ↓ Export PDF
            </button>
          </div>
          {completedLoading ? <Sk h="80px" /> : completed.length === 0 ? (
            <div className="mp-card" style={{ textAlign:'center', padding:40, color:'var(--text-faint)' }}>
              ...
              <div style={{ fontSize:12, color:'var(--text-faint)' }}>Service sheet submissions will appear here once submitted via QR scan or the Forms page.</div>
            </div>
          ) : (
            <div style={{ background:'var(--surface)', border:'1px solid var(--border)', overflow:'hidden' }}>
              {/* Table header */}
              <div style={{ display:'grid', gridTemplateColumns:'120px 1fr 1fr 80px 1fr 100px', gap:0, background:'var(--surface-2)', padding:'8px 14px', borderBottom:'1px solid var(--border)' }}>
              ...
              <div style={{ maxHeight:480, overflowY:'auto' }}>
                {completed.map((s, i) => (
                  <div key={s.id} style={{ display:'grid', gridTemplateColumns:'120px 1fr 1fr 80px 1fr 100px', gap:0, padding:'10px 14px', borderBottom: i < completed.length-1 ? '1px solid var(--border)' : 'none', background: i%2===0 ? 'transparent' : 'var(--surface-2)', alignItems:'start', transition:'background .1s' }}
                    ...
                    <div style={{ fontSize:12, fontWeight:600, color:'var(--text-primary)' }}>
                      {s.service_type || 'Service Sheet'}
                    </div>
                    <div style={{ fontSize:12, color:'var(--text-secondary)' }}>
                      {s.technician || '—'}
                    </div>
                    <div style={{ fontSize:12, color:'var(--text-secondary)', fontFamily:'var(--font-mono)' }}>
                      {s.total_labour_hours ? s.total_labour_hours + ' hrs' : '—'}
                    </div>
                    ...
                    <div>
                      <button onClick={() => {
                        const parts = s.parts?.map(p => `${p.name} x${p.qty||1}`).join(', ') || 'None';
                        const labour = s.labour?.map(l => `${l.description} (${l.hours}hrs)`).join(', ') || 'None';
                        alert(`Service: ${s.service_type||'Service Sheet'}
Date: ${new Date(s.created_at).toLocaleDateString('en-AU')}
Technician: ${s.technician||'—'}
Parts: ${parts}
Labour: ${labour}
Notes: ${s.notes||'—'}`);
                      }} style={{ padding:'4px 10px', background:'var(--surface-2)', border:'1px solid var(--border)', fontSize:11, color:'var(--text-secondary)', cursor:'pointer', fontFamily:'inherit' }}>
                        View
                      </button>
                    </div>
                  </div>
              ...
              <div style={{ padding:'8px 14px', background:'var(--surface-2)', borderTop:'1px solid var(--border)', fontSize:11, color:'var(--text-faint)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <span>{completed.length} service record{completed.length !== 1 ? 's' : ''} found</span>
