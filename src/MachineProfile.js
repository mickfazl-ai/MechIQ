...
function PrestartsTab({ asset, userRole }) {
  ...
  const loadBranding = async () => {
    const { data } = await supabase.from('company_branding').select('*').eq('company_id', asset.company_id).maybeSingle();
    setCompanyBranding(data);
  };

  const exportAllPDF = () => {
    const rows = prestarts.map(p => `<tr>
      <td>${new Date(p.created_at).toLocaleDateString('en-AU')}</td>
      <td>${p.operator_name||'—'}</td>
      <td>${p.hrs_start ? p.hrs_start.toLocaleString()+' hrs' : '—'}</td>
      <td style="color:${p.defects_found?'#B91C1C':'#15803D'};font-weight:600;">${p.defects_found ? 'Defects found' : 'Pass'}</td>
      <td>${p.notes||'—'}</td>
    </tr>`).join('');
    const win = window.open('', '_blank');
    win.document.write(`<!DOCTYPE html><html><head><title>Prestart History — ${asset.name}</title>
    <style>body{font-family:Inter,sans-serif;padding:32px;}h2{font-size:18px;margin-bottom:4px;}p{color:#64748b;font-size:12px;margin-bottom:20px;}table{width:100%;border-collapse:collapse;font-size:12px;}th{background:#f1f5f9;padding:8px 12px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:#64748b;border-bottom:2px solid #e5e7eb;}td{padding:8px 12px;border-bottom:1px solid #f1f5f9;}.footer{margin-top:32px;font-size:10px;color:#94a3b8;}</style>
    </head><body>
    <h2>Prestart History — ${asset.name}</h2>
    <p>${prestarts.length} records · Exported ${new Date().toLocaleDateString('en-AU')}</p>
    <table><thead><tr><th>Date</th><th>Operator</th><th>Hours</th><th>Result</th><th>Notes</th></tr></thead>
    <tbody>${rows}</tbody></table>
    <div class="footer">Generated by MechIQ · mechiq.com.au</div>
    </body></html>`);
    win.document.close();
    setTimeout(() => win.print(), 400);
  };

  const viewPrestart = (p) => {
    const b = companyBranding || {};
    const responses = p.responses || {};
    const logoHtml = b.logo_url
      ? `<img src="${b.logo_url}" style="height:48px;object-fit:contain;" alt="logo"/>`
      : `<div style="width:52px;height:52px;background:#0F172A;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#fff;">LOGO</div>`;
    const checkRows = Object.entries(responses).map(([item, result]) => {
      const pass = !String(result).toLowerCase().match(/fail|no\b|defect|unsatisf/);
      return `<div style="display:flex;align-items:center;gap:10px;padding:7px 10px;border-bottom:0.5px solid #e5e7eb;">
        <div style="width:13px;height:13px;border:0.5px solid ${pass?'#86EFAC':'#FCA5A5'};background:${pass?'#F0FDF4':'#FEF2F2'};display:flex;align-items:center;justify-content:center;font-size:9px;color:${pass?'#15803D':'#B91C1C'};flex-shrink:0;">${pass?'✓':'✕'}</div>
        <div style="flex:1;font-size:12px;color:#0F172A;">${item}</div>
        <div style="font-size:10px;font-weight:600;color:${pass?'#15803D':'#B91C1C'};">${result}</div>
      </div>`;
    }).join('') || '<div style="padding:10px;color:#94a3b8;font-size:12px;">No checklist responses recorded</div>';

    const win = window.open('', '_blank');
    win.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/>
<title>Prestart — ${asset.name} · ${new Date(p.created_at).toLocaleDateString('en-AU')}</title>
<style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Inter,-apple-system,sans-serif;background:#f1f5f9;padding:24px;}@media print{body{background:#fff;padding:0;}.no-print{display:none!important;}}.page{background:#fff;max-width:780px;margin:0 auto;padding:36px 40px;}.header{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:18px;border-bottom:2px solid #1976D2;margin-bottom:20px;}.banner{background:#F8FAFC;border:0.5px solid #e5e7eb;padding:14px 16px;margin-bottom:20px;display:grid;grid-template-columns:repeat(4,1fr);gap:12px;}.ab-l{font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.6px;margin-bottom:3px;}.ab-v{font-size:13px;font-weight:600;color:#0F172A;}.sec{margin-bottom:20px;}.sec-t{font-size:9px;font-weight:700;color:#1976D2;text-transform:uppercase;letter-spacing:.8px;margin-bottom:10px;padding-bottom:5px;border-bottom:0.5px solid #BFDBFE;display:flex;align-items:center;gap:6px;}.sec-t::before{content:'';width:3px;height:10px;background:#1976D2;flex-shrink:0;}.fg{display:grid;grid-template-columns:1fr 1fr;gap:10px;}.f{padding:8px 10px;background:#F8FAFC;border:0.5px solid #e5e7eb;}.fl{font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;}.fv{font-size:12px;font-weight:600;color:#0F172A;}.checks{border:0.5px solid #e5e7eb;overflow:hidden;}.defect-banner{background:#FEF2F2;border:0.5px solid #FCA5A5;border-left:3px solid #B91C1C;padding:10px 14px;margin-bottom:16px;}.pass-banner{background:#F0FDF4;border:0.5px solid #86EFAC;border-left:3px solid #15803D;padding:10px 14px;margin-bottom:16px;}.nb{background:#F8FAFC;border:0.5px solid #e5e7eb;padding:10px 12px;font-size:12px;color:#374151;line-height:1.6;min-height:48px;}.sig{background:#F8FAFC;border:0.5px solid #e5e7eb;padding:14px 16px;display:grid;grid-template-columns:1fr 1fr;gap:24px;}.sl{font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px;margin-bottom:12px;}.sline{border-bottom:1px solid #0F172A;height:36px;display:flex;align-items:flex-end;padding-bottom:3px;}.footer{border-top:0.5px solid #e5e7eb;padding-top:14px;margin-top:20px;display:flex;justify-content:space-between;align-items:center;}.no-print{display:flex;gap:10px;justify-content:center;margin:24px auto;max-width:780px;}.btn{padding:10px 24px;border:none;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;}</style>
</head><body>
<div class="page">
  <div class="header">
    <div style="display:flex;align-items:center;gap:12px;">${logoHtml}
      <div>
        <div style="font-size:16px;font-weight:700;color:#0F172A;">${b.company_name||'Your Company'}</div>
        <div style="font-size:11px;color:#64748b;margin-top:2px;">${b.address||asset.location||''}</div>
        ${b.abn?`<div style="font-size:10px;color:#94a3b8;">ABN ${b.abn}</div>`:''}
      </div>
    </div>
    <div style="text-align:right;">
      <div style="font-size:20px;font-weight:700;color:#1976D2;margin-bottom:4px;">Daily Prestart</div>
      <span style="display:inline-flex;align-items:center;gap:4px;padding:3px 9px;font-size:10px;font-weight:700;background:${p.defects_found?'#FEF2F2':'#F0FDF4'};color:${p.defects_found?'#B91C1C':'#15803D'};border:0.5px solid ${p.defects_found?'#FCA5A5':'#86EFAC'};">${p.defects_found?'⚠ Defects found':'✓ Pass'}</span>
      <div style="font-size:11px;color:#64748b;margin-top:4px;">Date: ${new Date(p.created_at).toLocaleDateString('en-AU')}</div>
      <div style="font-size:11px;color:#64748b;">Ref: PS-${String(p.id||'').slice(-5).padStart(5,'0')}</div>
    </div>
  </div>

  <div class="banner">
    <div><div class="ab-l">Asset</div><div class="ab-v">${asset.name}</div></div>
    <div><div class="ab-l">Asset ID</div><div class="ab-v" style="font-family:monospace;color:#1976D2;">${asset.asset_number||asset.id}</div></div>
    <div><div class="ab-l">Hours at prestart</div><div class="ab-v" style="font-family:monospace;">${p.hrs_start ? p.hrs_start.toLocaleString()+' hrs' : '—'}</div></div>
    <div><div class="ab-l">Location</div><div class="ab-v">${p.site_area||asset.location||'—'}</div></div>
  </div>

  ${p.defects_found
    ? `<div class="defect-banner"><div style="font-size:12px;font-weight:700;color:#B91C1C;margin-bottom:2px;">⚠ Defects identified — work order may have been raised</div><div style="font-size:11px;color:#7F1D1D;">Review checklist items marked as failed below.</div></div>`
    : `<div class="pass-banner"><div style="font-size:12px;font-weight:700;color:#15803D;">✓ All checks satisfactory — asset cleared for operation</div></div>`
  }

  <div class="sec">
    <div class="sec-t">Operator details</div>
    <div class="fg">
      <div class="f"><div class="fl">Operator</div><div class="fv">${p.operator_name||'—'}</div></div>
      <div class="f"><div class="fl">Date</div><div class="fv">${p.date||new Date(p.created_at).toLocaleDateString('en-AU')}</div></div>
      <div class="f"><div class="fl">Start hours</div><div class="fv" style="font-family:monospace;">${p.hrs_start ? p.hrs_start.toLocaleString()+' hrs' : '—'}</div></div>
      <div class="f"><div class="fl">Site area</div><div class="fv">${p.site_area||asset.location||'—'}</div></div>
    </div>
  </div>

  <div class="sec">
    <div class="sec-t">Prestart checklist</div>
    <div class="checks">${checkRows}</div>
  </div>

  <div class="sec">
    <div class="sec-t">Notes &amp; defects</div>
    <div class="nb">${p.notes||'No notes recorded.'}</div>
  </div>

  <div class="sec">
    <div class="sec-t">Signatures</div>
    <div class="sig">
      <div>
        <div class="sl">Operator signature</div>
        <div class="sline"><span style="font-size:11px;font-style:italic;color:#64748b;">${p.operator_signature?'Signed — '+(p.operator_name||''):'Not signed'}</span></div>
        <div style="font-size:10px;color:#94a3b8;margin-top:4px;">${p.operator_name||'—'} · ${new Date(p.created_at).toLocaleDateString('en-AU')}</div>
      </div>
      <div>
        <div class="sl">Supervisor sign-off</div>
        <div class="sline"></div>
        <div style="font-size:10px;color:#94a3b8;margin-top:4px;">Name &amp; date</div>
      </div>
    </div>
  </div>
  ...
  <div class="footer">
    <div style="display:flex;align-items:center;gap:8px;">
      <div style="width:20px;height:20px;background:#1976D2;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#fff;">M</div>
      <div style="font-size:11px;font-weight:700;color:#374151;">MECH<span style="color:#1976D2;">IQ</span></div>
      <div style="font-size:10px;color:#94a3b8;margin-left:4px;">· mechiq.com.au</div>
    </div>
    <div style="text-align:right;font-size:10px;color:#94a3b8;line-height:1.6;">Generated by MechIQ<br>${new Date().toLocaleDateString('en-AU')} · ${asset.name}</div>
  </div>
</div>
<div class="no-print">
  <button class="btn" style="background:#1976D2;color:#fff;" onclick="window.print()">Print / Save as PDF</button>
  <button class="btn" style="background:#F8FAFC;border:0.5px solid #e5e7eb;" onclick="window.close()">Close</button>
</div>
</body></html>`);
    win.document.close();
  };
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
        <div style={{ fontSize:13, fontWeight:700, color:'var(--text-secondary)' }}>
          Prestart history for <strong style={{ color:'var(--text-primary)' }}>{asset.name}</strong>
        </div>
        <button onClick={exportAllPDF}
          style={{ padding:'7px 14px', background:'var(--accent)', color:'#fff', border:'none', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
          ↓ Export PDF
        </button>
      </div>
      {loading ? <Sk h="80px" /> : prestarts.length === 0 ? (
        <div className="mp-card" style={{ textAlign:'center', padding:40, color:'var(--text-faint)' }}>
          <div style={{ fontSize:28, marginBottom:10 }}>📋</div>
          <div style={{ fontSize:14, fontWeight:700, color:'var(--text-secondary)', marginBottom:6 }}>No prestarts recorded</div>
          <div style={{ fontSize:12, color:'var(--text-faint)' }}>Prestart submissions will appear here once submitted via QR scan.</div>
        </div>
      ) : (
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)' }}>
          <div style={{ display:'grid', gridTemplateColumns:'120px 160px 110px 80px 1fr 100px', padding:'8px 14px', background:'var(--surface-2)', borderBottom:'1px solid var(--border)' }}>
            {['Date','Operator','Hours','Result','Notes','Actions'].map(h => (
              <div key={h} style={{ fontSize:10, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.5px' }}>{h}</div>
            ))}
          </div>
          <div style={{ maxHeight:500, overflowY:'auto' }}>
            {prestarts.map((p, i) => (
              <div key={p.id} style={{ display:'grid', gridTemplateColumns:'120px 160px 110px 80px 1fr 100px', padding:'10px 14px', borderBottom: i < prestarts.length-1 ? '1px solid var(--border)' : 'none', background: i%2===0?'transparent':'var(--surface-2)', alignItems:'start' }}>
                <div style={{ fontSize:12, color:'var(--text-secondary)', fontFamily:'var(--font-mono)' }}>{new Date(p.created_at).toLocaleDateString('en-AU')}</div>
                <div style={{ fontSize:12, fontWeight:600, color:'var(--text-primary)' }}>{p.operator_name||'—'}</div>
                <div style={{ fontSize:12, color:'var(--text-secondary)', fontFamily:'var(--font-mono)' }}>{p.hrs_start ? p.hrs_start.toLocaleString()+' hrs' : '—'}</div>
                <div>
                  <span style={{ fontSize:10, fontWeight:700, padding:'2px 7px', background: p.defects_found?'var(--red-bg)':'var(--green-bg)', color: p.defects_found?'var(--red)':'var(--green)', border: '1px solid '+(p.defects_found?'var(--red-border)':'var(--green-border)') }}>
                    {p.defects_found ? 'Defects' : 'Pass'}
                  </span>
                </div>
                <div style={{ fontSize:11, color:'var(--text-muted)', lineHeight:1.4 }}>{p.notes||'—'}</div>
                <div>
                  <button onClick={() => viewPrestart(p)}
                    style={{ padding:'4px 10px', background:'var(--accent)', color:'#fff', border:'none', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                    View PDF
                  </button>
                </div>
              </div>
          ...
          <div style={{ padding:'8px 14px', background:'var(--surface-2)', borderTop:'1px solid var(--border)', fontSize:11, color:'var(--text-faint)', display:'flex', justifyContent:'space-between' }}>
            <span>{prestarts.length} prestart record{prestarts.length!==1?'s':''}</span>
            <button onClick={loadPrestarts} style={{ background:'none', border:'none', fontSize:11, color:'var(--accent)', cursor:'pointer', fontFamily:'inherit' }}>↻ Refresh</button>
          </div>
        </div>
      )}
    </div>
...
function ServiceTab({ asset }) {
  ...
  const [serviceSubTab, setServiceSubTab] = useState('schedules');
  ...
  useEffect(() => {
    if (serviceSubTab === 'completed') { loadCompleted(); loadBranding(); }
  }, [serviceSubTab]);
  ...
  const loadCompleted = async () => {
    ...
    const { data } = await supabase.from('service_sheet_submissions').select('*')
      .or(`asset.eq.${asset.name},asset_id.eq.${asset.id}`)
      .eq('company_id', asset.company_id)
      .order('created_at', { ascending: false });
    setCompleted(data || []);
  ...
  const exportAllPDF = () => {
    const rows = completed.map(s => `<tr><td>${new Date(s.created_at).toLocaleDateString('en-AU')}</td><td>${s.service_type||'—'}</td><td>${s.technician||'—'}</td><td>${s.total_labour_hours ? s.total_labour_hours+' hrs' : '—'}</td><td>${s.notes||'—'}</td></tr>`).join('');
    const win = window.open('', '_blank');
    win.document.write(`<!DOCTYPE html><html><head><title>Service History — ${asset.name}</title><style>body{font-family:Inter,sans-serif;padding:32px;}table{width:100%;border-collapse:collapse;font-size:12px;}th{background:#f1f5f9;padding:8px 12px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:.5px;color:#64748b;border-bottom:2px solid #e5e7eb;}td{padding:8px 12px;border-bottom:1px solid #f1f5f9;}</style></head><body><h2 style="font-size:18px;margin-bottom:4px;">Service History — ${asset.name}</h2><p style="color:#64748b;font-size:12px;margin-bottom:20px;">${completed.length} records · Exported ${new Date().toLocaleDateString('en-AU')}</p><table><thead><tr><th>Date</th><th>Service</th><th>Technician</th><th>Labour</th><th>Notes</th></tr></thead><tbody>${rows}</tbody></table></body></html>`);
    win.document.close(); setTimeout(()=>win.print(),400);
  };

  const viewServiceSheet = (s) => {
    const b = companyBranding || {};
    const checks = Array.isArray(s.responses) ? s.responses : Object.entries(s.responses||{}).map(([label,value])=>({label,value}));
    const parts = s.parts||[];
    const labourHrs = s.total_labour_hours || (s.labour||[]).reduce((sum,l)=>sum+(parseFloat(l.hours)||0),0) || 0;
    const partsTotal = parts.reduce((sum,p)=>sum+((parseFloat(p.cost)||0)*(parseInt(p.qty)||1)),0);
    const logoHtml = b.logo_url ? `<img src="${b.logo_url}" style="height:48px;object-fit:contain;" alt="logo"/>` : `<div style="width:52px;height:52px;background:#0F172A;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:#fff;">LOGO</div>`;
    const checkRows = checks.length > 0 ? checks.map(ch=>{
      const val = ch.value||ch.result||'Satisfactory';
      const pass = !String(val).toLowerCase().match(/fail|defect|no\b|unsatisf/);
      return `<div style="display:flex;align-items:center;gap:10px;padding:7px 10px;border-bottom:0.5px solid #e5e7eb;"><div style="width:13px;height:13px;border:0.5px solid ${pass?'#86EFAC':'#FCA5A5'};background:${pass?'#F0FDF4':'#FEF2F2'};display:flex;align-items:center;justify-content:center;font-size:9px;color:${pass?'#15803D':'#B91C1C'};flex-shrink:0;">${pass?'✓':'✕'}</div><div style="flex:1;font-size:12px;color:#0F172A;">${ch.label||ch.item||'Check'}</div><div style="font-size:10px;font-weight:600;color:${pass?'#15803D':'#B91C1C'};">${val}</div></div>`;
    }).join('') : '<div style="padding:10px;color:#94a3b8;font-size:12px;">No checklist items recorded</div>';
    const partsRows = parts.length > 0 ? parts.map(p=>`<tr><td>${p.name||'—'}</td><td style="font-family:monospace;font-size:11px;">${p.part_id||'—'}</td><td>${p.qty||1}</td><td>$${(parseFloat(p.cost)||0).toFixed(2)}</td><td>$${((parseFloat(p.cost)||0)*(parseInt(p.qty)||1)).toFixed(2)}</td></tr>`).join('') : '<tr><td colspan="5" style="color:#94a3b8;text-align:center;padding:10px;">No parts recorded</td></tr>';
    const win = window.open('', '_blank');
    win.document.write(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"/><title>Service Sheet — ${asset.name}</title>
<style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Inter,-apple-system,sans-serif;background:#f1f5f9;padding:24px;}@media print{body{background:#fff;padding:0;}.no-print{display:none!important;}}.page{background:#fff;max-width:780px;margin:0 auto;padding:36px 40px;}.header{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:18px;border-bottom:2px solid #1976D2;margin-bottom:20px;}.banner{background:#F8FAFC;border:0.5px solid #e5e7eb;padding:14px 16px;margin-bottom:20px;display:grid;grid-template-columns:repeat(4,1fr);gap:12px;}.ab-l{font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.6px;margin-bottom:3px;}.ab-v{font-size:13px;font-weight:600;color:#0F172A;}.sec{margin-bottom:20px;}.sec-t{font-size:9px;font-weight:700;color:#1976D2;text-transform:uppercase;letter-spacing:.8px;margin-bottom:10px;padding-bottom:5px;border-bottom:0.5px solid #BFDBFE;display:flex;align-items:center;gap:6px;}.sec-t::before{content:'';width:3px;height:10px;background:#1976D2;flex-shrink:0;}.fg{display:grid;grid-template-columns:1fr 1fr;gap:10px;}.f{padding:8px 10px;background:#F8FAFC;border:0.5px solid #e5e7eb;}.fl{font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;}.fv{font-size:12px;font-weight:600;color:#0F172A;}table{width:100%;border-collapse:collapse;}th{background:#F8FAFC;padding:7px 10px;font-size:9px;font-weight:700;color:#94a3b8;text-align:left;border:0.5px solid #e5e7eb;text-transform:uppercase;letter-spacing:.4px;}td{padding:7px 10px;font-size:11px;color:#374151;border:0.5px solid #e5e7eb;}tr:nth-child(even) td{background:#F8FAFC;}.totals{display:flex;justify-content:flex-end;gap:24px;padding:10px 12px;background:#F8FAFC;border:0.5px solid #e5e7eb;margin-top:8px;}.tl{font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;margin-bottom:2px;text-align:right;}.tv{font-size:15px;font-weight:700;color:#1976D2;font-family:monospace;text-align:right;}.nb{background:#F8FAFC;border:0.5px solid #e5e7eb;padding:10px 12px;font-size:12px;color:#374151;line-height:1.6;min-height:48px;}.sig{background:#F8FAFC;border:0.5px solid #e5e7eb;padding:14px 16px;display:grid;grid-template-columns:1fr 1fr;gap:24px;}.sl{font-size:9px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px;margin-bottom:12px;}.sline{border-bottom:1px solid #0F172A;height:36px;display:flex;align-items:flex-end;padding-bottom:3px;}.footer{border-top:0.5px solid #e5e7eb;padding-top:14px;margin-top:20px;display:flex;justify-content:space-between;align-items:center;}.no-print{display:flex;gap:10px;justify-content:center;margin:24px auto;max-width:780px;}.btn{padding:10px 24px;border:none;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;}</style></head><body>
<div class="page">
<div class="header">
  <div style="display:flex;align-items:center;gap:12px;">${logoHtml}<div><div style="font-size:16px;font-weight:700;color:#0F172A;">${b.company_name||'Your Company'}</div><div style="font-size:11px;color:#64748b;margin-top:2px;">${b.address||asset.location||''}</div>${b.abn?`<div style="font-size:10px;color:#94a3b8;">ABN ${b.abn}</div>`:''}</div></div>
  <div style="text-align:right;"><div style="font-size:20px;font-weight:700;color:#1976D2;margin-bottom:4px;">Service Sheet</div><span style="display:inline-flex;align-items:center;gap:4px;padding:3px 9px;font-size:10px;font-weight:700;background:#F0FDF4;color:#15803D;border:0.5px solid #86EFAC;">✓ Completed</span><div style="font-size:11px;color:#64748b;margin-top:4px;">Date: ${new Date(s.created_at).toLocaleDateString('en-AU')}</div><div style="font-size:11px;color:#64748b;">Ref: SS-${String(s.id||'').slice(-5).padStart(5,'0')}</div></div>
</div>
<div class="banner">
  <div><div class="ab-l">Asset</div><div class="ab-v">${asset.name}</div></div>
  <div><div class="ab-l">Asset ID</div><div class="ab-v" style="font-family:monospace;color:#1976D2;">${asset.asset_number||asset.id}</div></div>
  <div><div class="ab-l">Hours at service</div><div class="ab-v" style="font-family:monospace;">${asset.hours?asset.hours.toLocaleString()+' hrs':'—'}</div></div>
  <div><div class="ab-l">Service type</div><div class="ab-v">${s.service_type||'Service Sheet'}</div></div>
</div>
<div class="sec"><div class="sec-t">Service details</div><div class="fg"><div class="f"><div class="fl">Technician</div><div class="fv">${s.technician||'—'}</div></div><div class="f"><div class="fl">Location</div><div class="fv">${asset.location||'—'}</div></div><div class="f"><div class="fl">Labour hours</div><div class="fv" style="font-family:monospace;">${labourHrs} hrs</div></div><div class="f"><div class="fl">Service date</div><div class="fv">${s.date||new Date(s.created_at).toLocaleDateString('en-AU')}</div></div></div></div>
${checks.length>0?`<div class="sec"><div class="sec-t">Service checklist</div><div style="border:0.5px solid #e5e7eb;">${checkRows}</div></div>`:''}
<div class="sec"><div class="sec-t">Parts used</div><table><thead><tr><th>Part name</th><th>Part #</th><th>Qty</th><th>Unit cost</th><th>Total</th></tr></thead><tbody>${partsRows}</tbody></table><div class="totals"><div><div class="tl">Parts</div><div class="tv">$${partsTotal.toFixed(2)}</div></div><div><div class="tl">Labour (${labourHrs}hrs)</div><div class="tv">$${(labourHrs*100).toFixed(2)}</div></div><div><div class="tl">Total</div><div class="tv">$${(partsTotal+labourHrs*100).toFixed(2)}</div></div></div></div>
<div class="sec"><div class="sec-t">Notes</div><div class="nb">${s.notes||'No notes recorded.'}</div></div>
<div class="sec"><div class="sec-t">Signatures</div><div class="sig"><div><div class="sl">Technician signature</div><div class="sline"><span style="font-size:11px;font-style:italic;color:#64748b;">${s.operator_signature?'Signed — '+(s.technician||''):'Not signed'}</span></div><div style="font-size:10px;color:#94a3b8;margin-top:4px;">${s.technician||'—'} · ${new Date(s.created_at).toLocaleDateString('en-AU')}</div></div><div><div class="sl">Supervisor sign-off</div><div class="sline"></div><div style="font-size:10px;color:#94a3b8;margin-top:4px;">Name &amp; date</div></div></div></div>
<div class="footer"><div style="display:flex;align-items:center;gap:8px;"><div style="width:20px;height:20px;background:#1976D2;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;color:#fff;">M</div><div style="font-size:11px;font-weight:700;color:#374151;">MECH<span style="color:#1976D2;">IQ</span></div><div style="font-size:10px;color:#94a3b8;margin-left:4px;">· mechiq.com.au</div></div><div style="text-align:right;font-size:10px;color:#94a3b8;line-height:1.6;">Generated by MechIQ<br>${new Date().toLocaleDateString('en-AU')} · ${asset.name}</div></div>
</div>
<div class="no-print"><button class="btn" style="background:#1976D2;color:#fff;" onclick="window.print()">Print / Save as PDF</button><button class="btn" style="background:#F8FAFC;border:0.5px solid #e5e7eb;" onclick="window.close()">Close</button></div>
</body></html>`);
    win.document.close();
  };
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', borderBottom:'1px solid var(--border)', marginBottom:16 }}>
        {[['schedules','Schedules'],['completed','Completed']].map(([id,label]) => (
          <button key={id} onClick={() => setServiceSubTab(id)}
            style={{ padding:'9px 18px', fontSize:12, fontWeight:serviceSubTab===id?700:500,
              color:serviceSubTab===id?'var(--accent)':'var(--text-muted)',
              background:'none', border:'none', borderBottom:serviceSubTab===id?'2px solid var(--accent)':'2px solid transparent',
      ...
      {/* ── Completed sub-tab ── */}
      {serviceSubTab === 'completed' && (
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
            <div style={{ fontSize:13, fontWeight:700, color:'var(--text-secondary)' }}>
              Service completion history for <strong style={{ color:'var(--text-primary)' }}>{asset.name}</strong>
            </div>
            <button onClick={exportAllPDF}
              style={{ padding:'7px 14px', background:'var(--accent)', color:'#fff', border:'none', fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
              ↓ Export PDF
          ...
          {completedLoading ? <Sk h="80px" /> : completed.length === 0 ? (
            <div className="mp-card" style={{ textAlign:'center', padding:40, color:'var(--text-faint)' }}>
              <div style={{ fontSize:28, marginBottom:10 }}>📋</div>
              <div style={{ fontSize:14, fontWeight:700, color:'var(--text-secondary)', marginBottom:6 }}>No completed services yet</div>
              <div style={{ fontSize:12, color:'var(--text-faint)' }}>Service sheet submissions will appear here once submitted via QR scan.</div>
            </div>
          ) : (
            <div style={{ background:'var(--surface)', border:'1px solid var(--border)' }}>
              <div style={{ display:'grid', gridTemplateColumns:'120px 1fr 140px 80px 1fr 100px', padding:'8px 14px', background:'var(--surface-2)', borderBottom:'1px solid var(--border)' }}>
                {['Date','Service','Technician','Labour','Notes','Actions'].map(h => (
                  <div key={h} style={{ fontSize:10, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.5px' }}>{h}</div>
                ))}
              </div>
              <div style={{ maxHeight:480, overflowY:'auto' }}>
                {completed.map((s, i) => (
                  <div key={s.id} style={{ display:'grid', gridTemplateColumns:'120px 1fr 140px 80px 1fr 100px', padding:'10px 14px', borderBottom: i < completed.length-1 ? '1px solid var(--border)' : 'none', background: i%2===0?'transparent':'var(--surface-2)', alignItems:'start' }}>
                    <div style={{ fontSize:12, color:'var(--text-secondary)', fontFamily:'var(--font-mono)' }}>{new Date(s.created_at).toLocaleDateString('en-AU')}</div>
                    <div style={{ fontSize:12, fontWeight:600, color:'var(--text-primary)' }}>{s.service_type||'Service Sheet'}</div>
                    <div style={{ fontSize:12, color:'var(--text-secondary)' }}>{s.technician||'—'}</div>
                    <div style={{ fontSize:12, color:'var(--text-secondary)', fontFamily:'var(--font-mono)' }}>{s.total_labour_hours ? s.total_labour_hours+' hrs' : '—'}</div>
                    <div style={{ fontSize:11, color:'var(--text-muted)', lineHeight:1.4 }}>{s.notes||'—'}</div>
                    <div style={{ display:'flex', gap:5 }}>
                      <button onClick={() => viewServiceSheet(s)}
                        style={{ padding:'4px 10px', background:'var(--accent)', color:'#fff', border:'none', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
                        View PDF
                      </button>
                    </div>
              ...
              <div style={{ padding:'8px 14px', background:'var(--surface-2)', borderTop:'1px solid var(--border)', fontSize:11, color:'var(--text-faint)', display:'flex', justifyContent:'space-between' }}>
                <span>{completed.length} service record{completed.length!==1?'s':''}</span>
                <button onClick={loadCompleted} style={{ background:'none', border:'none', fontSize:11, color:'var(--accent)', cursor:'pointer', fontFamily:'inherit' }}>↻ Refresh</button>
              </div>
            </div>
          )}
        </div>
...
{ id:'prestarts',    label:'Prestarts' },
  ...
  return (
    <div style={{ animation:'fadeUp 0.4s ease both' }}>
      ...
      {activeTab === 'prestarts'    && <PrestartsTab asset={asset} userRole={userRole} />}
