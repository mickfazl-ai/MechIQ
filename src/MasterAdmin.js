  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#a0b0b0', margin: '4px 0 0', fontSize: '13px' }}>
          Manage company registrations, features and access
        </p>
      </div>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '14px', flexWrap: 'wrap', alignItems: 'center' }}>
            {['all', 'pending', 'active', 'suspended'].map(t => (
              <button key={t} onClick={() => setTab(t)} style={{
                ...
                backgroundColor: tab === t ? '#00c2e0' : '#1a2f2f',
                color: tab === t ? '#000' : '#a0b0b0'
              }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ color: '#a0b0b0', fontSize: '12px' }}>
                        {c.industry} · {c.contact_name} · {c.phone}
                      </div>
                    ...
                    <div style={{ display: 'flex', gap: '6px', flexShrink: 0, marginLeft: '10px' }}>
                      {c.status === 'pending' && (
                        <>
                          <button style={btnStyle('#00c264')} onClick={e => { e.stopPropagation(); setStatus(c.id, 'active'); }}>✓ Approve</button>
                          <button style={btnStyle('#e94560')} onClick={e => { e.stopPropagation(); setStatus(c.id, 'suspended'); }}>✕ Reject</button>
                        </>
                      )}
                      {c.status === 'active' && (
                        <button style={btnStyle('#e94560')} onClick={e => { e.stopPropagation(); setStatus(c.id, 'suspended'); }}>Suspend</button>
                      )}
                      {c.status === 'suspended' && (
                        <button style={btnStyle('#00c264')} onClick={e => { e.stopPropagation(); setStatus(c.id, 'active'); }}>Reactivate</button>
                      )}
                    </div>
                  </div>
                  <div style={{ marginTop: '8px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    {FEATURES.map(f => (
                      <span key={f.key} style={{
                        ...
                        backgroundColor: c.features?.[f.key] !== false ? '#0a2a1a' : '#2a0a0a',
                        color: c.features?.[f.key] !== false ? '#00c264' : '#e94560'
                      }}>{f.label}</span>
                    ))}
                  </div>
                </div>
          ...
          <div style={{ backgroundColor: '#0d1515', border: '1px solid #1a3a3a', borderRadius: '10px', padding: '20px', height: 'fit-content', position: 'sticky', top: '20px' }}>
            ...
            <div style={{ marginBottom: '16px', borderTop: '1px solid #1a2f2f', paddingTop: '14px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                <input
                  type="number"
                  value={selectedCompany.asset_limit || 10}
              ...
              {FEATURES.map(f => {
                ...
                return (
                  <div key={f.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    ...
                    <button
                      onClick={() => toggleFeature(selectedCompany, f.key)}
                      style={{
                        padding: '4px 12px', borderRadius: '12px', border: 'none', cursor: 'pointer',
                        fontSize: '11px', fontWeight: 600,
                        backgroundColor: enabled ? '#0a2a1a' : '#2a0a0a',
                        color: enabled ? '#00c264' : '#e94560'
                      }}
                    >
                      {enabled ? '✓ ON' : '✕ OFF'}
                    </button>
