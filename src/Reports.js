...
const CSS = `
  ...
  .r-input:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(25,118,210,0.12); }
  .r-btn {
    padding: 9px 18px; background: #1976D2; color: #fff; border: none;
    border-radius: 8px; font-size: 12px; font-weight: 700; cursor: pointer;
    font-family: inherit; letter-spacing: 0.4px; box-shadow: 0 3px 10px rgba(25,118,210,0.3);
    transition: all 0.15s; white-space: nowrap;
  }
  .r-btn:hover { background: #1565C0; transform: translateY(-1px); }
  .r-btn-ghost {
...
function Reports({ companyId, userRole, initialTab }) {
  ...
  const exportDowntimePDF = () => {
    ...
    const doc = new jsPDF();
    doc.setFillColor(240,246,252); doc.rect(0,0,210,297,'F');
    ...
    doc.setTextColor(30,50,70); doc.setFontSize(16); doc.setFont('helvetica','bold'); doc.text('DOWNTIME REPORT',14,40);
    ...
    doc.setFillColor(26,75,110); doc.rect(14,62,55,20,'F'); doc.rect(74,62,55,20,'F'); doc.rect(134,62,62,20,'F');
    ...
    doc.setTextColor(30,50,70); doc.setFontSize(16); doc.setFont('helvetica','bold');
    doc.text(String(filteredData.length),18,78); doc.text(filteredHours.toFixed(1)+'h',78,78);
    if (showCost) { doc.setTextColor(255,107,0); doc.text('$'+filteredCost.toLocaleString('en-AU',{minimumFractionDigits:2}),138,78); }
    ...
    else { autoTable(doc,{startY:98,head:[showCost ? ['Asset','Date','Category','Hours','Cost','Description'] : ['Asset','Date','Category','Hours','Description']],body:filteredData.map(d=>showCost ? [d.asset,d.date,d.category,d.hours+'h','$'+parseFloat(d.cost||0).toLocaleString('en-AU',{minimumFractionDigits:2}),d.description] : [d.asset,d.date,d.category,d.hours+'h',d.description]),theme:'plain',headStyles:{fillColor:[26,75,110],textColor:[255,255,255],fontSize:8,fontStyle:'bold'},bodyStyles:{fillColor:[255,255,255],textColor:[40,60,80],fontSize:9},alternateRowStyles:{fillColor:[245,250,255]},styles:{lineColor:[200,220,235],lineWidth:0.1}}); }
    ...
    autoTable(doc,{startY:finalY+18,head:[['Category','Hours Lost']],body:getCategoryTotals().map(c=>[c.category,c.hours+'h']),theme:'plain',headStyles:{fillColor:[26,75,110],textColor:[255,255,255],fontSize:8,fontStyle:'bold'},bodyStyles:{fillColor:[255,255,255],textColor:[40,60,80],fontSize:9},styles:{lineColor:[200,220,235],lineWidth:0.1}});
  ...
  const exportAvailabilityPDF = () => {
    ...
    doc.setTextColor(0,194,224); doc.setFontSize(28); doc.setFont('helvetica','bold'); doc.text('MECH IQ',14,20);
    doc.setTextColor(160,176,176); doc.setFontSize(10); doc.setFont('helvetica','normal'); doc.text('by Coastline Machine Management',14,27);
    doc.setTextColor(30,50,70); doc.setFontSize(16); doc.setFont('helvetica','bold'); doc.text('MACHINE AVAILABILITY REPORT',14,40);
    doc.setTextColor(160,176,176); doc.setFontSize(10); doc.setFont('helvetica','normal');
  ...
  const exportExcel = async () => {
    ...
    const eh=es.addRow(showCost ? ['Asset','Date','Start','End','Hours','Cost ($)','Category','Description','Reported By'] : ['Asset','Date','Start','End','Hours','Category','Description','Reported By']);eh.height=22;['A','B','C','D','E','F','G','H','I'].forEach(c=>colHeader(eh.getCell(c)));
    filteredData.forEach((d,i)=>{const r=es.addRow(showCost ? [d.asset,d.date,d.start_time,d.end_time,parseFloat(d.hours||0),parseFloat(d.cost||0),d.category,d.description,d.reported_by] : [d.asset,d.date,d.start_time,d.end_time,parseFloat(d.hours||0),d.category,d.description,d.reported_by]);r.height=18;['A','B','C','D'].forEach(col=>dataCell(r.getCell(col),i%2!==0));numCell(r.getCell('E'),i%2!==0,blue);numCell(r.getCell('F'),i%2!==0,'CC4400');['G','H','I'].forEach(col=>dataCell(r.getCell(col),i%2!==0));});
