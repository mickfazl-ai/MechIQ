...
function Register({ onBackToLogin }) {
  ...
  const handleSelectPlan = (plan) => {
    if (plan.id === 'enterprise') {
      window.location.href = 'mailto:michael@coastlinemm.com.au?subject=MaintainIQ Enterprise Enquiry';
      return;
    }
    ...
    try {
      ...
      const { data: company, error: companyError } = await supabase
        ...
        .insert({
          ...
          phone: form.phone,
          status: 'pending',
          asset_limit: selectedPlan.asset_limit,
          features,
      ...
      const { error: roleError } = await supabase
        .from('user_roles')
        .insert({ email: form.email, name: form.contactName, role: 'admin', company_id: company.id });
    ...
    <div className="login-page">
      <div className="login-card" style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: '#a0b0b0', marginBottom: '8px', fontSize: '14px' }}>
          Thanks <strong style={{ color: 'white' }}>{form.contactName}</strong>! Your account for{' '}
          <strong style={{ color: 'white' }}>{form.companyName}</strong> is pending approval.
        </p>
    ...
    <div className="login-page">
      <div className="login-card" style={{ maxWidth: '460px' }}>
        ...
        <div style={{
          ...
          backgroundColor: '#0d1515', border: `1px solid ${selectedPlan.color}`,
          ...
          <button onClick={() => { setStep('plans'); setFormStep(1); setError(''); }}
            style={{ background: 'transparent', border: 'none', color: '#a0b0b0', cursor: 'pointer', fontSize: '12px' }}>
            Change ↩
          </button>
        </div>

        <p className="login-subtitle">
          {formStep === 1 ? 'Company Details — Step 1 of 2' : 'Account Setup — Step 2 of 2'}
        </p>

        <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', justifyContent: 'center' }}>
          {[1, 2].map(s => (
            <div key={s} style={{ width: '32px', height: '4px', borderRadius: '2px', backgroundColor: s <= formStep ? selectedPlan.color : '#1a2f2f' }} />
          ))}
        </div>
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0a0f0f', padding: '40px 20px' }}>
      <div style={{ textAlign: 'center', marginBottom: '40px' }}>
        ...
        <p style={{ color: '#a0b0b0', margin: 0, fontSize: '14px' }}>
        ...
        {PLANS.map(plan => (
          <div key={plan.id} style={{
            ...
            <button
              ...
              style={{
                width: '100%', padding: '13px', border: 'none', borderRadius: '7px',
                backgroundColor: plan.color, color: plan.id === 'trial' ? '#000' : '#fff',
      ...
      <div style={{ textAlign: 'center' }}>
        <button onClick={onBackToLogin} style={{
          background: 'transparent', border: 'none', color: '#a0b0b0',
